	...
	table.render({
		...
		cols: [
			[{
				...
				field: 'title',title: '文章标题',templet: '#usernameTpl'
			}, {
				field: 'date',title: '拟稿人',sort: true
			}, {
				field: 'category',title: '显示情况',sort: true
			}, {
				field: 'sort',title: '发布时间',sort: true
			}, {
				field: 'top',title: '发布时间',templet: '#topTpl',unresize: true
			}, {
				field: 'review',title: '状态',templet: '#reviewTpl',unresize: true
			}, {
...
function zTreeOnClick(event, treeId, treeNode) {
	...
	var subjectId = treeNode.sbIndex
	console.log(22222,subjectId)
	layui.use('table', function () {
		var table = layui.table;
		var path = myFunction();
		$.ajax({
			type: "post",
			url: path+'/article/getArticleBySubjectId?subjectId='+subjectId,
				...
				table.render({
					...
					cols: [
						[{
							...
							field: 'acId', title: '文章ID', sort: true
						}, {
							field: 'acPrimaryTitle', title: '文章标题', templet: '#usernameTpl'
						}, {
							field: 'usName', title: '拟稿人', sort: true
						}, {
							field: '', title: '显示情况', sort: true
						}, {
							field: '', title: '发布时间', sort: true
						}, {
							field: '', title: '发布时间', templet: '#topTpl', unresize: true
						}, {
							field: '', title: '状态', templet: '#reviewTpl', unresize: true
						}, {
							field: 'operate', title: '操作', toolbar: '#operateTpl', unresize: true
						}]
					],
		...
		table.on('tool(demo)', function(obj){
			...
			if(obj.event === 'detail'){
				...
				$.ajax({
					type: "post",
					url: path+'/kafka/addMessage?acId='+ data.acId,
					success: function (res) {
